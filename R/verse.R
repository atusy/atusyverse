...
atusyverse_attach <- function() {
  ...
  msg(
    cli::rule(
      ...
      right = paste0("atusyverse ", package_version("atusyverse"))
    ),
    startup = TRUE
  )
...
#' Conflicts between the atusyverse and other packages
#'
#' This function lists all the conflicts between packages in the atusyverse
...
#' @export
#' @examples
#' atusyverse_conflicts()
atusyverse_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  pkg_names <- paste0("package:", atusyverse_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% pkg_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "atusyverse_conflicts")
}

atusyverse_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "atusyverse_conflicts()"
  )
...
#' @export
print.atusyverse_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(atusyverse_conflict_message(x))
}
...
#' atusyverse_update()
#' }
atusyverse_update <- function(recursive = FALSE) {

  deps <- atusyverse_deps(recursive)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All atusyverse packages up-to-date")
    return(invisible())
  }
...
#' List all atusyverse dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   atusyverse packages.
#' @export
atusyverse_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("atusyverse", pkgs, recursive = recursive)
...
#' List all packages in the atusyverse
#'
#' @param include_self Include atusyverse in the list?
#' @export
#' @examples
#' atusyverse_packages()
atusyverse_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("atusyverse")$Imports
  imports <- strsplit(raw, ",")[[1]]
  parsed <- gsub("^\\s+|\\s+$", "", imports)
  names <- vapply(strsplit(parsed, "\\s+"), "[[", 1, FUN.VALUE = character(1))

  if (include_self) {
    names <- c(names, "atusyverse")
  }
...
.onAttach <- function(...) {
  ...
  if (!"package:conflicted" %in% search()) {
    x <- atusyverse_conflicts()
    msg(atusyverse_conflict_message(x), startup = TRUE)
  }
